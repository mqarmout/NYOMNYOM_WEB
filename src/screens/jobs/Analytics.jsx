  ...
  return (
    <div style={{ padding: 18, display: "flex", flexDirection: "column", gap: 14, fontFamily: "var(--font-mono)" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-end", gap: 32 }}>
          {[
            ...
            <div key={l}>
              ...
              <div style={{ fontSize: 44, color: c, lineHeight: 0.95, textShadow: glowFn(theme, tweaks.glow * 1.2), marginTop: 4 }}>{v}</div>
            </div>
          ))}
        </div>
      </Box>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 14, alignItems: "stretch" }}>
            ...
            {pipelineCounts.length === 0 ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {pipelineCounts.map(({ status, count, color }) => (
                  <div key={status} style={{ display: "grid", gridTemplateColumns: "80px 1fr 26px", gap: 10, alignItems: "center" }}>
            ...
            {monthBars.every((d) => d.v === 0) ? (
              ...
              <svg viewBox="0 0 760 100" width="100%" height="100" preserveAspectRatio="none">
                ...
                {monthBars.map((d, i) => {
                  ...
                  return (
                    <g key={i}>
                      <rect x={x} y={80 - bH} width={bW} height={bH}
                        fill={d.hot ? theme.accentHot : theme.accent} opacity={d.hot ? 1 : 0.55} />
                      <text x={x + bW / 2} y={95} textAnchor="middle" fill={theme.muted} fontSize="9" fontFamily="monospace">{d.lbl}</text>
                    </g>
                  );
                })}
              </svg>
        ...
        {/* Recent applications */}
        <Box title="RECENT.APPLICATIONS" padding="14px 18px" style={{ display: "flex", flexDirection: "column" }}>
          <div style={{
            ...
            <span>date</span><span>company</span><span>role</span><span style={{ textAlign: "right" }}>status</span>
          </div>
          <div style={{ flex: 1, overflowY: "auto" }}>
            {recent.length === 0 ? (
              ...
              recent.map((j) => {
                ...
                return (
                  <div key={j.id} style={{
                    fontSize: 11, color: theme.cream, padding: "8px 0",
                    display: "grid", gridTemplateColumns: "56px 1fr 100px 80px",
                    ...
                    <span style={{ color: theme.accentHot, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {j.company}
                    </span>
                    <span style={{ color: theme.cream, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: 10 }}>
                      {j.role}
                    </span>
          ...
          {jobs.length > 0 && (
            <div style={{ marginTop: 10, fontSize: 10, color: theme.muted, borderTop: `1px dashed ${theme.border}`, paddingTop: 8 }}>
              {jobs.length} total · {active} active · {((active / Math.max(jobs.length, 1)) * 100).toFixed(0)}% active rate
            </div>
          )}
        </Box>
      </div>
