...
function extractFromText(text) {
  let role = null, company = null, salary = null;
  ...
  if (atM) { role = atM[1].trim(); company = atM[2].trim(); }
...
function UrlStepModal({ onSuccess, onFailed, onSkip, onClose }) {
  ...
  const handleExtract = async () => {
    ...
    setLoading(false);
    if (res.error) {
      onFailed(trimmed, 'The server couldn\'t fetch that URL — it may require a login or block automated access.');
      return;
    }
    ...
    <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal-box">
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="modal-save-btn" onClick={handleExtract} disabled={loading} style={{ flex: 1 }}>
      ...
      <div className="modal-box">
        ...
        {found.length > 0 ? (
          <>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 0, marginBottom: 20, border: '1px solid var(--border)' }}>
              {found.map(({ label, value }) => (
                <div key={label} style={{ display: 'flex', gap: 12, padding: '9px 12px', borderBottom: '1px solid var(--border)', fontSize: 13 }}>
                  <span style={{ color: 'var(--muted)', width: 60, flexShrink: 0, fontSize: 11 }}>{label}</span>
          ...
          <div style={{ fontSize: 13, color: 'var(--muted)', marginBottom: 20 }}>
            Nothing specific could be extracted{prefill.site_used ? ` from ${prefill.site_used}` : ''}. The URL has been saved — fill in the rest manually.
          </div>
        )}
  ...
  return (
    <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal-box">
        ...
        <div style={{
          fontSize: 12, color: 'var(--danger)', background: 'rgba(255,48,48,0.06)',
          border: '1px solid rgba(255,48,48,0.2)', padding: '10px 12px', marginBottom: 16,
        }}>
          {errorMsg}
        </div>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="modal-save-btn" onClick={handleContinue} style={{ flex: 1 }} disabled={!text.trim()}>
            Extract & Continue
          </button>
          <button
            className="modal-save-btn"
...
function JobModal({ initial, onSave, onClose }) {
  ...
  const [dateApplied, setDateApplied] = useState(initial?.date_applied || new Date().toISOString().slice(0, 10));
  ...
  return (
    <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal-box">
        ...
        <div className="field">
          <label>Company</label>
          <input type="text" placeholder="Company name" value={company} onChange={e => setCompany(e.target.value)} autoFocus={!initial?.id} />
        </div>
        <div className="field">
          <label>Role</label>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <div className="field">
            <label>Salary</label>
            <input type="text" placeholder="e.g. $80k – $100k" value={salary} onChange={e => setSalary(e.target.value)} />
          </div>
          <div className="field">
            <label>Site Used</label>
            <input type="text" placeholder="e.g. LinkedIn" value={siteUsed} onChange={e => setSiteUsed(e.target.value)} />
          </div>
        </div>
        <div className="field">
          <label>Job URL</label>
        ...
        <div className="field">
          ...
          <textarea
            ...
            rows={3}
            style={{ width: '100%', padding: '12px 14px', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 12, color: 'var(--text)', fontSize: 14, outline: 'none', resize: 'vertical' }}
          />
        </div>
...
export default function Applications() {
  ...
  const JobCard = ({ job }) => (
    <div className="job-card" onClick={() => setModal({ mode: 'edit', job })}>
      ...
      {job.url && (
        <a className="job-card-link" href={job.url} target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()}>
  ...
  return (
    <div className="screen">
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <h1>Applications</h1>
          <p>{jobs.length} total · {boardJobs.filter(j => j.status !== 'rejected').length} active</p>
        </div>
        <button
      ...
      {archivedJobs.length > 0 && (
        <>
          <div className="section-title" style={{ marginTop: 32 }}>Withdrawn</div>
