...
function StatTile({ label, value, sub, hot = false, warn = false }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontFamily: "var(--font-mono)", fontSize: 9, color: theme.muted, letterSpacing: "0.16em" }}>
        {label}
      </div>
      <div style={{
        fontFamily: "var(--font-mono)", fontSize: 24, color,
      ...
      {sub && (
        <div style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: hot ? theme.accent : theme.accentDim, marginTop: 4 }}>
          {sub}
        </div>
      )}
    </div>
...
function WeightMiniChart({ history, theme, tweaks }) {
  ...
  const weights = pts90.map(m => m.weight).filter(Boolean).reverse();
  ...
  return (
    <Box title="WEIGHT.90D" padding="14px 16px">
      <div style={{ fontFamily: "var(--font-mono)", fontSize: 18, color: theme.accentHot, textShadow: glowFn(theme, tweaks.glow * 0.6) }}>
        {latest.toFixed(1)}<span style={{ fontSize: 10, color: theme.accentDim }}> {unit}</span>
      </div>
      <div style={{ fontFamily: "var(--font-mono)", fontSize: 9, color: trendUp ? STATUS.amber : theme.accent, marginBottom: 4 }}>
      ...
      <svg viewBox="0 0 240 64" width="100%" height="56" preserveAspectRatio="none">
        ...
        <circle cx={pts[pts.length - 1][0]} cy={pts[pts.length - 1][1]} r="3" fill={theme.accentHot} />
      </svg>
    </Box>
  );
}
...
function RunMiniChart({ runHistory, theme, tweaks }) {
  ...
  return (
    <Box title="RUN.KM/WK" padding="14px 16px">
      <div style={{ fontFamily: "var(--font-mono)", fontSize: 18, color: theme.accentHot, textShadow: glowFn(theme, tweaks.glow * 0.6) }}>
        {latest.total_km.toFixed(1)}<span style={{ fontSize: 10, color: theme.accentDim }}> km</span>
      </div>
      <div style={{ fontFamily: "var(--font-mono)", fontSize: 9, color: theme.accent, marginBottom: 4 }}>
      ...
      <svg viewBox="0 0 240 64" width="100%" height="56" preserveAspectRatio="none">
        {weeks.map((w, i) => {
          ...
          return (
            <rect
              key={i}
              x={i * 20 + 4} y={56 - h} width="14" height={h}
...
function ClimbMiniChart({ climbs, theme, tweaks }) {
  ...
  return (
    <Box title="CLIMB.GRADES" padding="14px 16px">
      <div style={{ fontFamily: "var(--font-mono)", fontSize: 18, color: theme.accentHot, textShadow: glowFn(theme, tweaks.glow * 0.6) }}>
        {totalSent}<span style={{ fontSize: 10, color: theme.accentDim }}> sent · {topGrade} max</span>
      </div>
      <div style={{ fontFamily: "var(--font-mono)", fontSize: 9, color: theme.accent, marginBottom: 6 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
          {grades.map((g, i) => (
            <div key={g} style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <span style={{ fontFamily: "var(--font-mono)", fontSize: 9, color: theme.muted, width: 22 }}>{g}</span>
              <div style={{
                height: 7, width: `${(gradeMap[g] / maxVal) * 100}%`,
                background: gradeColors[i] || theme.accentDim,
                boxShadow: `0 0 5px ${(gradeColors[i] || theme.accentDim)}80`,
                minWidth: 4,
              }} />
              <span style={{ fontFamily: "var(--font-mono)", fontSize: 9, color: theme.muted }}>{gradeMap[g]}</span>
            </div>
          ))}
        </div>
      ) : (
        <div style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: theme.muted }}>no boulder data</div>
      )}
    </Box>
  );
}

function PortfolioShowcase({ projects, onNav, theme, tweaks }) {
  ...
  return (
    <Box title={`PORTFOLIO · PUBLIC`} padding="16px 20px">
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
        <span style={{ width: 7, height: 7, background: theme.accent, boxShadow: `0 0 6px ${theme.accent}` }} />
        ...
        <button
          ...
          style={{
            ...
            border: `1px solid ${theme.borderHi}`, cursor: "pointer", letterSpacing: "0.06em",
          }}
        >
          manage ›
        </button>
        ...
        {displayProjects.map((p) => {
          ...
          return (
            <div key={p.id} style={{
              ...
              <div style={{ height: 52, position: "relative", borderBottom: `1px solid ${theme.border}` }}>
                <svg viewBox="0 0 200 52" width="100%" height="100%" preserveAspectRatio="none">
                  ...
                  <text x="100" y="38" textAnchor="middle" fontSize="28"
                    fontFamily="JetBrains Mono, monospace"
                    fill={theme.accentDim} fontWeight="700" opacity="0.6"
                  >{initial}</text>
                </svg>
                <span style={{
                ...
                }}>● {isLive ? "LIVE" : "WIP"}</span>
              </div>
              <div style={{ padding: "8px 10px" }}>
                <div style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: theme.cream, fontWeight: 600 }}>
                  {p.title.length > 14 ? p.title.slice(0, 13) + "…" : p.title}
                </div>
                <div style={{ fontFamily: "var(--font-mono)", fontSize: 9, color: theme.accent, marginTop: 2 }}>
...
function AlertsPanel({ expenses, categories, hydroLatest, jobs, theme, tweaks }) {
  ...
  const alerts = useMemo(() => {
    ...
    categories.forEach(cat => {
      ...
      const spent = expenses.filter(e => e.category_id === cat.id && e.date?.startsWith(mo)).reduce((s, e) => s + e.amount, 0);
      if (spent > cat.budget) list.push({ c: STATUS.red, sym: "!", label: cat.name, msg: `+$${(spent - cat.budget).toFixed(0)} over` });
    });
    // Hydro out of range
    if (hydroLatest?.ph != null && (hydroLatest.ph < 5.8 || hydroLatest.ph > 6.4)) {
      list.push({ c: STATUS.amber, sym: "!", label: "hydro", msg: `pH ${hydroLatest.ph.toFixed(1)} out of range` });
    }
    if (hydroLatest?.water_level != null && hydroLatest.water_level < 30) {
      list.push({ c: STATUS.red, sym: "!", label: "hydro", msg: `water low ${hydroLatest.water_level.toFixed(0)}%` });
    }
    // Job interviews
    const interviewing = jobs.filter(j => j.status === "interviewing");
    if (interviewing.length > 0) {
      list.push({ c: STATUS.amber, sym: "?", label: interviewing[0].company || "job", msg: "interview active" });
    }
    // Offers
    const offers = jobs.filter(j => j.status === "offer");
    if (offers.length > 0) {
      list.push({ c: theme.accentHot, sym: "!", label: offers[0].company || "offer", msg: "pending decision" });
    }
    // OK status
  ...
  return (
    <Box title="ALERTS" padding="14px 16px">
      <div style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: theme.cream, lineHeight: 1.8 }}>
        {alerts.map((a, i) => (
          <div key={i}>
            [<span style={{ color: a.c }}>{a.sym}</span>] {a.label.padEnd(8)} <span style={{ color: a.c }}>{a.msg}</span>
          </div>
        ))}
      </div>
    </Box>
...
function ReplPanel({ expenses, workouts, climbs, runs, theme, tweaks }) {
  const entries = useMemo(() => {
    const list = [];
    const today = new Date().toISOString().slice(0, 10);

    // Today's expenses
    expenses.filter(e => e.date === today).slice(0, 3).forEach(e => {
      list.push({ t: "spend", m: `${e.merchant || "expense"} · $${e.amount.toFixed(2)}` });
    });
    // Today's workouts
    workouts.filter(w => w.date === today).slice(0, 2).forEach(w => {
      list.push({ t: "fit", m: `${w.name || "workout"} · ${w.duration ? w.duration + " min" : "—"}` });
    });
    // Today's climbs
    climbs.filter(c => c.date === today).slice(0, 2).forEach(c => {
      list.push({ t: "climb", m: `${c.name || "climb"} · ${c.sent ? "sent" : "attempt"}` });
    });
    // Today's runs
    runs.filter(r => r.date === today).slice(0, 1).forEach(r => {
      list.push({ t: "run", m: `${r.distance_km?.toFixed(1)}km run` });
    });

    list.sort(() => Math.random() - 0.5);
    return list.length ? list : [
      { t: "sys", m: "no activity logged today" },
    ];
  }, [expenses, workouts, climbs, runs]);

  return (
    <Box title="REPL" padding="14px 16px" style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
      <div style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: theme.cream, lineHeight: 1.8, flex: 1, overflow: "auto" }}>
        <div><span style={{ color: theme.accent }}>$</span> cat today.log</div>
        <div style={{ color: theme.muted }}>{"─".repeat(25)}</div>
        {entries.map((e, i) => (
          <div key={i}>
            <span style={{ color: theme.accent }}>{e.t.padEnd(5)}</span>{"  "}{e.m}
          </div>
        ))}
        <div style={{ color: theme.muted }}>{"─".repeat(25)}</div>
        <div>
...
export default function Home({ username, onNav }) {
  ...
  const runKm = runs.filter(r => r.date?.startsWith(mo)).reduce((s, r) => s + (r.distance_km || 0), 0);
  ...
  // Sections grid
  const SECTION_META = [
    { id: "spending", ic: "coins", key: "1", label: "SPENDING",
      ...
      <div style={{
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14, minWidth: 0 }}>
          ...
          <Box glowing padding="20px 22px">
            ...
            <div style={{
              fontSize: isPhone ? 22 : 30, color: theme.accentHot, marginTop: 6,
              lineHeight: 1.1, textShadow: glowFn(theme, tweaks.glow * 1.2),
            }}>
            ...
            <div style={{ fontSize: 12, color: theme.cream, marginTop: 6 }}>
              ...
              {interviewing > 0 && <> · <span style={{ color: STATUS.amber }}>{interviewing} interview{interviewing > 1 ? "s" : ""}</span></>}
              {" · "}<span style={{ color: theme.accent }}>{currency}{totalSpent.toFixed(0)}</span> spent
              {" · last sync "}<span style={{ color: theme.accent }}>just now</span>
            </div>
          </Box>
          ...
          <div style={{ display: "grid", gridTemplateColumns: isPhone ? "repeat(2, 1fr)" : "repeat(4, 1fr)", gap: 10 }}>
            <StatTile
              ...
              sub={vsPrev !== null ? `${vsPrev >= 0 ? "+" : ""}${vsPrev}% vs last mo` : `${budgetPct.toFixed(0)}% used`}
            ...
            <StatTile
              ...
              value={hydroLatest ? `pH ${hydroLatest.ph?.toFixed(1) ?? "—"}` : activeJobs.length || "—"}
          ...
          <Box title="SPENDING · 30D" padding="18px 20px">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 10 }}>
              <div>
                <div style={{ fontSize: 20, color: theme.accentHot, textShadow: glowFn(theme, tweaks.glow * 0.8) }}>
                  {currency}{totalSpent.toFixed(0)}
                  {totalBudget > 0 && <span style={{ color: theme.muted, fontSize: 12 }}> / {currency}{totalBudget.toFixed(0)}</span>}
                </div>
                <div style={{ fontSize: 10, color: theme.accentDim, marginTop: 3 }}>
                  {budgetPct.toFixed(0)}% used · {daysLeft}d remaining
                  {vsPrev !== null && ` · ${Number(vsPrev) <= 0 ? "↓" : "↑"}${Math.abs(vsPrev)}% vs last mo`}
                </div>
              </div>
              <button
                ...
                style={{
                  ...
                  background: "transparent", color: theme.cream,
                  border: `1px solid ${theme.borderHi}`, cursor: "pointer",
                }}
              >
                open ›
              </button>
          ...
          {portfolioProjects.length > 0 && (
            <PortfolioShowcase projects={portfolioProjects} onNav={onNav} theme={theme} tweaks={tweaks} />
          )}

          {/* Sections grid */}
          <Box title="SECTIONS" padding="16px 20px">
            <div style={{ display: "grid", gridTemplateColumns: isPhone ? "repeat(2, 1fr)" : isTablet ? "repeat(4, 1fr)" : "repeat(auto-fit, minmax(150px, 1fr))", gap: 8 }}>
              {SECTION_META.map(s => (
                <button
                  key={s.id}
                  onClick={() => onNav(s.id)}
                  style={{
                    ...
                    color: theme.cream, textAlign: "left", ...mono, cursor: "pointer",
                    position: "relative",
                  }}
                  onMouseEnter={e => { e.currentTarget.style.borderColor = theme.accent; e.currentTarget.style.background = theme.surface2; }}
                  onMouseLeave={e => { e.currentTarget.style.borderColor = s.hot ? theme.accent : theme.borderHi; e.currentTarget.style.background = theme.surface; }}
                >
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <span style={{ color: theme.muted, fontSize: 10 }}>[{s.key}]</span>
                    <span style={{ flex: 1 }} />
                    {s.hot && <span style={{ width: 6, height: 6, background: theme.accent, boxShadow: `0 0 6px ${theme.accent}` }} />}
                  </div>
                  <div style={{ fontSize: 11, color: theme.cream, marginTop: 8, letterSpacing: "0.06em" }}>{s.label}</div>
                  <div style={{ fontSize: 16, color: s.hot ? theme.accentHot : theme.cream, marginTop: 4, textShadow: s.hot ? glowFn(theme, tweaks.glow * 0.6) : "none" }}>{s.v}</div>
        ...
        {!isPhone && !isTablet && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14, minHeight: 0 }}>
            <AlertsPanel
              ...
              hydroLatest={hydroLatest} jobs={jobs}
              theme={theme} tweaks={tweaks}
            />
            <ReplPanel
              expenses={expenses} workouts={workouts}
              climbs={climbs} runs={runs}
              theme={theme} tweaks={tweaks}
            />
          </div>
        )}
            ...
            <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
              {[
                ...
                <div key={k}>
                  ...
                  <div style={{ fontSize: 22, color: ok ? theme.accentHot : STATUS.amber, lineHeight: 1.1 }}>{v}</div>
                </div>
              ))}
            </div>
          </Box>
