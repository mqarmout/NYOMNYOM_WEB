  ...
  return (
    <div className="public-page">
      <header className="public-header">
        ...
        <button className="public-signin-btn" onClick={() => setShowSignIn(true)}>Sign In</button>
      </header>
      ...
      <div className="public-container">
        ...
        <section className="public-hero">
          ...
          {about.location  && <p className="public-location"><Px name="map-pin" size={11} /> {about.location}</p>}
          {about.bio       && <p className="public-bio">{about.bio}</p>}
          <div className="public-links">
            {about.website  && <a href={about.website}                                   target="_blank" rel="noreferrer"><Px name="globe" size={13} /> Website</a>}
            {about.github   && <a href={'https://github.com/' + about.github}             target="_blank" rel="noreferrer">GitHub</a>}
            {about.linkedin && <a href={'https://linkedin.com/in/' + about.linkedin}      target="_blank" rel="noreferrer">LinkedIn</a>}
          </div>
        </section>
              ...
              {Object.keys(grouped).sort().map(cat => (
                <div className="skills-group" key={cat}>
                  ...
                  <div className="skill-chips">
                    {grouped[cat].map(s => (
                      <div className="skill-chip public-chip" key={s.id}>{s.name}</div>
                    ))}
                  </div>
                </div>
              ))}
      ...
      {showSignIn && (
        <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) setShowSignIn(false); }}>
