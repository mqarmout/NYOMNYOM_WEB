...
function MetricChart({ title, data, color, unit }) {
  ...
  return (
    <div className="graph-card" style={{ marginBottom: 20 }}>
      ...
      <div style={{ textAlign: 'right', fontSize: 11, color: 'var(--fg-dim)', marginTop: 4 }}>{unit}</div>
    </div>
  );
}
    ...
    <div className="screen">
      ...
      {history.length < 2 ? (
        ...
        <>
          ...
          <MetricChart title="EC (ppm)" data={make('ec_ppm')} color="var(--accent)" unit="nutrient concentration" />
          <MetricChart title="Water Temperature (°C)" data={make('water_temp')} color="#5a9ed4" unit="target range 18–24°C" />
          <MetricChart title="Water Level (%)" data={make('water_level')} color="#2eb8a0" unit="reservoir fill level" />
          <MetricChart title="Air Temperature (°C)" data={make('air_temp')} color="#d4a040" unit="ambient" />
          <MetricChart title="Humidity (%)" data={make('humidity')} color="#b87ad4" unit="relative humidity" />

          <div style={{ marginTop: 16 }}>
            <div className="graph-card-title" style={{ marginBottom: 10 }}>ALL READINGS</div>
