...
export default function SpendingHero({ data, expenses, categories, profile }) {
  ...
  const total = data?.total ?? expenses?.filter(e => e.date?.startsWith(mo)).reduce((s, e) => s + e.amount, 0) ?? 0;
  const effBudget = data?.effective_budget || categories?.reduce((s, c) => s + (c.budget || 0), 0) || 0;
  const baseBudget = data?.base_budget || effBudget;
  ...
  const overBudget = effBudget > 0 && total > effBudget;
  ...
  return (
    <>
      <Box glowing padding="18px 22px">
        ...
        <div style={{ display: "flex", alignItems: "flex-end", gap: 22, marginTop: 6, flexWrap: "wrap" }}>
          <div style={{
            ...mono, fontSize: 48, color: theme.accentHot, lineHeight: 0.95,
            textShadow: glowFn(theme, tweaks.glow * 1.6),
          }}>
            {currency}{Math.floor(total).toLocaleString()}
          ...
          <div style={{ ...mono, fontSize: 12, color: theme.cream, lineHeight: 1.6 }}>
            {effBudget > 0 && (
              <div>
                of{" "}
                <span style={{ color: theme.accentHot }}>{fmt(effBudget, currency)}</span>
                {rolloverIn > 0 && (
                  <span style={{ color: theme.accentDim, fontSize: 10 }}>{" "}(+{fmt(rolloverIn, currency)} rollover)</span>
                )}
                {" "}budget
              </div>
            )}
          ...
          <div style={{ flex: 1 }} />
          <div style={{ display: "flex", flexDirection: "column", gap: 6, alignSelf: "flex-start", marginTop: 8 }}>
            <button
              onClick={() => setShowAdd(true)}
              style={{
                ...mono, fontSize: 11, padding: "8px 16px",
