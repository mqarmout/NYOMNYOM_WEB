...
export default function SpendingHistory() {
  ...
  const totalOut = filtered.filter(tx => tx._type === "expense").reduce((s, tx) => s + tx.amount, 0);
  const totalIn = filtered.filter(tx => tx._type === "income").reduce((s, tx) => s + tx.amount, 0);
  ...
  return (
    <div style={{ padding: 18, display: "flex", flexDirection: "column", gap: 14, ...mono }}>
      ...
      <Box glowing padding="14px 22px">
        <div style={{ fontSize: 10, color: theme.muted, letterSpacing: "0.18em", marginBottom: 10 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: 14 }}>
          {[
            ...
            ["NET", (net >= 0 ? "+" : "") + fmt(Math.abs(net), currency), net >= 0 ? theme.accent : STATUS.red],
      ...
      {/* Filters + list */}
      <Box title="ALL.TRANSACTIONS" padding="14px 18px" style={{ display: "flex", flexDirection: "column" }}>

        {/* Filter bar */}
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
          <div style={{
            display: "flex", alignItems: "center", gap: 8, flex: 1, minWidth: 160,
            padding: "6px 10px", background: theme.surface2, border: `1px solid ${theme.border}`,
          }}>
            ...
            <input
              ...
              style={{ ...mono, fontSize: 11, color: theme.cream, background: "none", border: "none", outline: "none", flex: 1 }}
            />
          </div>
          ...
          {["all", "expense", "income"].map(t => (
            <button key={t}
              ...
              style={{
                ...mono, fontSize: 10, padding: "6px 12px", letterSpacing: "0.08em",
          ...
          <select
            ...
            style={{
              ...mono, fontSize: 10, padding: "6px 10px 6px 10px",
              background: theme.surface2, border: `1px solid ${theme.border}`,
              color: theme.cream, cursor: "pointer", outline: "none",
            }}
          >
        ...
        <div style={{ overflowY: "auto", maxHeight: "calc(100vh - 360px)", minHeight: 200 }}>
          {loading ? (
            <div style={{ ...mono, fontSize: 12, color: theme.muted, padding: "12px 0" }}>loading…</div>
          ) : filtered.length === 0 ? (
            <div style={{ ...mono, fontSize: 12, color: theme.muted, padding: "12px 0" }}>no entries match</div>
          ) : filtered.map(tx => {
            ...
            return (
              <div
                ...
                style={{
                  ...mono, fontSize: 11, color: isIncome ? theme.accent : theme.cream,
                  padding: "6px 0",
                  display: "grid", gridTemplateColumns: "70px 1fr 90px 80px",
              ...
              >
                ...
                <span style={{ color: isIncome ? theme.accent : theme.accentHot, textAlign: "right" }}>
        ...
        {/* Footer totals */}
        <div style={{
          marginTop: 8, paddingTop: 8, borderTop: `1px dashed ${theme.border}`,
          ...mono, fontSize: 10, color: theme.muted,
          display: "flex", gap: 20, flexWrap: "wrap",
        }}>
          <span>{filtered.length} entries</span>
          <span>out: <span style={{ color: theme.accentHot }}>{fmt(totalOut, currency)}</span></span>
          <span>in: <span style={{ color: theme.accent }}>{fmt(totalIn, currency)}</span></span>
          <span>net: <span style={{ color: net >= 0 ? theme.accent : STATUS.red }}>
      ...
      {modal?.type === "expense" && (
        <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) handleClose(); }}>
          <div className="modal-box"><AddExpense onClose={handleClose} initial={modal.item} /></div>
        </div>
      )}
      {modal?.type === "income" && (
        <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) handleClose(); }}>
          <div className="modal-box"><AddIncome onClose={handleClose} initial={modal.item} /></div>
        </div>
      )}
    </div>
  );
