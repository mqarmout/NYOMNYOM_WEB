  ...
  return (
    <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal-box">
        ...
        <div className="field">
          <label>Icon</label>
          <input type="text" placeholder="Search icons…" value={iconSearch}
            onChange={e => setIconSearch(e.target.value)} style={{ marginBottom: 8 }} />
          <div className={styles.iconPicker}>
            {filteredIcons.length === 0
              ? <div style={{ fontSize: 11, color: "var(--muted)", padding: "6px 2px" }}>No icons match</div>
              : filteredIcons.map(ic => (
                <button key={ic} className={`${styles.iconOpt}${icon === ic ? " selected" : ""}`}
        ...
        <div className="field">
          <label>Name</label>
          <input type="text" placeholder="e.g. Food & Drink" value={name}
            onChange={e => setName(e.target.value)} autoFocus={!initial} />
        </div>
        <div className="field">
          <label>Monthly Budget (optional)</label>
          <input type="number" min="0" step="0.01" placeholder="Leave blank for no limit"
...
export default function Categories() {
  const { categories, expenses, profile, addCategory, updateCategory, deleteCategory } = useApp();
    ...
    <div style={{ padding: 18, display: "flex", flexDirection: "column", gap: 14, ...mono }}>
      ...
      <Box title={`ALL.CATEGORIES · ${categories.length}`} padding="16px 22px">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
          {catsWithSpent.length === 0 ? (
            ...
            return (
              ...
              >
                <span><Px name={c.icon} size={14} /></span>
                <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  {c.name}
                </span>
                <span style={{ color: over ? STATUS.red : theme.accent }}>
                  {"█".repeat(blocks)}
                  <span style={{ color: theme.faint }}>{"░".repeat(Math.max(0, 24 - blocks))}</span>
                </span>
                ...
                <div style={{ display: "flex", gap: 4, justifyContent: "flex-end" }}>
                  <button
                    ...
                    style={{
                      ...mono, fontSize: 10, color: theme.accent,
        ...
        <div style={{
          ...
          ...mono, fontSize: 12, color: theme.accentHot,
        }}>
          <span></span>
          <span>TOTAL</span>
          <span></span>
          <span style={{ color: theme.muted }}>{fmt(totalSpent, currency)} / {fmt(totalBudget, currency)}</span>
