...
export default function Dashboard() {
  ...
  const filtered =
    txFilter === "income"
      ? allTx.filter((t) => t._type === "income")
      : txFilter === "expenses"
        ? allTx.filter((t) => t._type === "expense")
        : allTx;
      ...
      <div className="page-header">
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button
            ...
            onClick={() => setModal({ type: "expense", item: null })}
          >
            + Expense
          </button>
        </div>
      ...
      <div className="tx-list">
        {filtered.length === 0 ? (
          ...
          filtered.map((tx) =>
            tx._type === "income" ? (
              <div
                className="tx-item tx-item-income"
                key={"i" + tx.id}
                onClick={() => setModal({ type: "income", item: tx })}
                style={{ cursor: "pointer" }}
              >
                <div className="tx-icon">
                  <Px name={SOURCE_ICONS[tx.source] || "plus"} size={16} />
                </div>
                <div className="tx-info">
                  <div className="tx-name">
                    {tx.description}
                    {tx.recurring ? (
                      <span className="tx-recurring-badge">{tx.recurring_period}</span>
                    ) : null}
                  </div>
                  <div className="tx-cat">
                    {tx.source && tx.source !== "other"
              ...
              <div
                className="tx-item"
                key={"e" + tx.id}
                onClick={() => setModal({ type: "expense", item: tx })}
                style={{ cursor: "pointer" }}
              >
                <div className="tx-icon">
                  <Px name={tx.category_icon} size={16} />
                </div>
                <div className="tx-info">
                  <div className="tx-name">
                    {tx.description}
