...
import { Px, IClose, IEdit, IChevDown, IChevUp, IWarning, IBoard, ICal, IExtLink, IRefresh } from '../../icons';
...
const EMPTY = {
  ...
  tech_stack: '', github_url: '', live_url: '', start_date: today(), notes: '',
};
      ...
      <div className="modal-box" style={{ width: 580 }}>
        ...
        <div className="field">
          ...
          <input value={form.name} onChange={e => set('name', e.target.value)}
            placeholder="NYOMNYOM, my-cli-tool..." autoFocus />
        </div>
        ...
        <div className="field">
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {STATUSES.map(s => (
              <button key={s} className={`chip ${form.status === s ? 'active' : 'inactive'}`}
                style={{ flex: 1 }} onClick={() => set('status', s)}>
                {STATUS_LABELS[s]}
              </button>
            ))}
          </div>
        ...
        <div className="field">
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {PRIORITIES.map(p => (
              <button key={p} className={`chip ${form.priority === p ? 'active' : 'inactive'}`}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 16px' }}>
          <div className="field">
            ...
            <input value={form.tech_stack} onChange={e => set('tech_stack', e.target.value)}
        ...
        <button className="modal-save-btn" onClick={handleSave} disabled={saving || !form.name.trim()}>
...
function ProjectCard({ project, onEdit, onDelete, onOpenBoard }) {
  ...
  const techChips = (project.tech_stack || '').split(',').map(t => t.trim()).filter(Boolean);
    ...
    <div className="devp-card">
      <div className="devp-card-header" onClick={() => setExpanded(e => !e)}>
        <div className="devp-card-header-left">
          ...
          <span className={`devp-status-badge s-${project.status}`}>{STATUS_LABELS[project.status]}</span>
          <span className={`devp-priority-badge p-${project.priority}`}>{PRIORITY_LABELS[project.priority]}</span>
          {isStale && <span className="devp-stale-badge"><IWarning /> STALE</span>}
        </div>
        <div className="devp-card-header-right">
          <button className="devp-board-btn"
            onClick={e => { e.stopPropagation(); onOpenBoard(project); }}
            title="Open kanban board">
            <IBoard /> BOARD
          </button>
          <span className="devp-chevron">{expanded ? <IChevUp /> : <IChevDown />}</span>
          <button className="climb-card-action"
            onClick={e => { e.stopPropagation(); onEdit(); }} title="Edit"><IEdit /></button>
          <button className="climb-card-action danger"
            onClick={e => { e.stopPropagation(); onDelete(); }} title="Delete"><IClose /></button>
        </div>
      </div>
        ...
        <div className="devp-card-body">
          ...
          {techChips.length > 0 && (
            <div className="devp-tech-chips">
              {techChips.map(t => <span key={t} className="devp-tech-chip">{t}</span>)}
            </div>
          )}

          <div className="devp-meta">
            {daysActive !== null && <span><ICal /> {daysActive}d active</span>}
            {project.github_url && (
              <a href={project.github_url} target="_blank" rel="noreferrer"
                className="devp-link" onClick={e => e.stopPropagation()}>GitHub <IExtLink /></a>
            )}
            {project.live_url && (
              <a href={project.live_url} target="_blank" rel="noreferrer"
                className="devp-link" onClick={e => e.stopPropagation()}>Live <IExtLink /></a>
            )}
          </div>
          ...
          {project.github_url && (
            <div className="devp-commit">
              ...
              {!fetchingCommit && !commit && (
                <button className="devp-fetch-btn" onClick={e => {
                  e.stopPropagation();
                  setFetchingCommit(true);
                  fetchCommit(project.id).finally(() => setFetchingCommit(false));
                }}><IRefresh /> fetch commit</button>
              )}
            </div>
          )}
...
export default function DevProjects() {
  const {
    projects, kanbanTasks, loadAll,
    addProject, updateProject, deleteProject,
    addKanbanTask, updateKanbanTask, deleteKanbanTask,
  } = useDevProjects();
  ...
  return (
    <div className="screen">
      <div className="page-header">
        ...
        <button className="sidebar-add-btn" onClick={() => setModal('new')}>+ NEW PROJECT</button>
      </div>
      ...
      {filtered.length === 0
        ? <div className="empty-state">No projects here.<br />Click + NEW PROJECT to get started.</div>
