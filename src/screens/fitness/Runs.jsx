    ...
    >
      <div className="modal-box">
        ...
        <div className="field">
          ...
          <input
            type="text"
            placeholder="e.g. Morning run, Parkrun"
            value={title}
...
export default function Runs() {
  ...
  useEffect(() => {
    ...
    if (strava === "connected") {
      ...
      setToast({
        type: "ok",
        msg: `Strava connected! Imported ${count} run${count === "1" ? "" : "s"}.`,
      });
      loadAll();
  ...
  const handleStravaImport = useCallback(async () => {
    ...
    if (res.error) {
      ...
      setToast({
        type: "ok",
        msg:
          n > 0 ? `Imported ${n} new run${n === 1 ? "" : "s"} from Strava.` : "Already up to date.",
      });
      if (n > 0) loadAll();
    }
  }, [loadAll]);
  ...
  const chartData = runHistory.map((r) => ({
    v: Math.round(r.total_km * 10) / 10,
    lbl: r.week.slice(5),
  }));
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center", marginTop: 4 }}>
          {stravaStatus &&
            ...
            (stravaStatus.configured ? (
              ...
              <span
                title="Set STRAVA_CLIENT_ID and STRAVA_CLIENT_SECRET on the server"
                style={{
                  padding: "10px 18px",
                  borderRadius: 8,
                  fontSize: 13,
          ...
          {stravaStatus?.connected && (
            <button
              ...
              style={{
                ...
                fontSize: 13,
                fontWeight: 600,
                background: importing ? "#a33000" : "#fc4c02",
                color: "#fff",
                border: "none",
      ...
      <div className="finance-cards">
        <div className="finance-card income-card">
          ...
          <div className="fc-amount">
            {totalKm.toFixed(1)} <span style={{ fontSize: 16 }}>km</span>
          </div>
          <div className="fc-sub">
            {thisMonth.length} run{thisMonth.length !== 1 ? "s" : ""}
          </div>
        </div>
        <div className="finance-card spent-card">
          <div className="fc-label">Time</div>
          <div className="fc-amount" style={{ fontSize: 22 }}>
        ...
        <div className="finance-card net-card net-positive">
          ...
          <div className="fc-amount" style={{ fontSize: 20 }}>
            {avgPaceSecs ? fmtPace(avgPaceSecs) : "--"}
          </div>
      ...
      <div className="tx-list">
        {runs.length === 0 ? (
          <div className="empty-state">
            No runs logged yet.
            <br />
            Click <strong>+ Log Run</strong> to get started.
          </div>
        ) : (
          runs.map((r) => (
            <div className="tx-item" key={r.id}>
              <div className="tx-icon">
                <span
                  style={{
                    fontSize: 14,
                    fontWeight: 700,
                    color: TYPE_COLORS[r.run_type] || "var(--accent)",
                  }}
                >
              ...
              <div className="tx-info">
                ...
                <div className="tx-cat">
                  {r.distance_km.toFixed(2)} km · {fmtDuration(r.duration_seconds)}
                </div>
              </div>
              <div className="tx-right">
                <div className="tx-amount" style={{ fontSize: 14 }}>
                  {fmtPace(r.pace_seconds_per_km)}
                </div>
                <div className="tx-date">{fmtDate(r.date)}</div>
              </div>
              <button className="tx-delete" onClick={() => setEditing(r)}>
                <IEdit />
              </button>
              <button className="tx-delete" onClick={() => deleteRun(r.id)}>
                <IClose />
              </button>
            </div>
          ))
