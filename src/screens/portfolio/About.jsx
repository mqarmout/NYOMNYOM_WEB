...
export default function About() {
  ...
  const [form, setForm] = useState({ display_name: '', headline: '', bio: '', location: '', website: '', github: '', linkedin: '' });
    ...
    <div className="screen">
      ...
      <div className="about-layout">
        <div className="about-preview">
          <div className="profile-avatar" style={{ margin: '0 auto 16px' }}>{getInitials(form.display_name || '?')}</div>
          ...
          <div className="about-links">
            {form.website  && <a href={form.website}                           target="_blank" rel="noreferrer">🌐 Website</a>}
            {form.github   && <a href={'https://github.com/' + form.github}    target="_blank" rel="noreferrer">GitHub</a>}
            {form.linkedin && <a href={'https://linkedin.com/in/' + form.linkedin} target="_blank" rel="noreferrer">LinkedIn</a>}
          </div>
        </div>
        ...
        <div className="settings-card">
          ...
          {[
            ...
            <div className="settings-row" key={f.key}>
              ...
              <input className="settings-row-input" type="text" placeholder={f.placeholder}
                value={form[f.key]} onChange={set(f.key)} />
            </div>
          ))}
          <div className="field" style={{ marginTop: 16 }}>
            <label>Bio</label>
            <textarea placeholder="A short bio about yourself..." value={form.bio} onChange={set('bio')} rows={4}
          ...
          <button className="save-btn" onClick={() => saveAbout(form)}>Save</button>
        </div>
      </div>
    </div>
  );
