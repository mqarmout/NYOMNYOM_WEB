...
const SECTIONS = {
  ...
  climbing: {
    ...
    items: [
      { id: 'climbing-routes', label: 'ROUTES' },
    ],
  },
  projects: {
    accent:      '#40c4c4',
  ...
  useEffect(() => {
    ...
    const handler = (e) => {
      ...
      if (e.key === 'Enter') {
        ...
        if (hit) { onNavigate(hit.screen, hit.fire); onClose(); }
        return;
      }
      if (e.key === 'Tab') {
        e.preventDefault();
      ...
      <div className="modal-box shortcuts-help">
        ...
        <table className="shortcuts-table">
          <tbody>
            {[
              ...
              <tr key={key}>
                <td><span className="sk">{key}</span></td>
                <td>{desc}</td>
              </tr>
            ))}
          </tbody>
        ...
        <div className="shortcuts-sections">
          {SECTION_ORDER.map((s, i) => (
            <span key={s}><span className="sk">{i + 1}</span> {s.charAt(0).toUpperCase() + s.slice(1)}{i < SECTION_ORDER.length - 1 ? '  ·  ' : ''}</span>
          ))}
        </div>
      </div>
    </div>
    ...
    <div id="app" style={{
      ...
      <div className="world-layer" style={getWorldZoom(sec)}>
        ...
        <svg className="mountain-bg" viewBox="0 0 1000 600" preserveAspectRatio="none">
          ...
          <polygon points="0,600 0,520 160,430 340,475 520,390 720,450 900,400 1000,440 1000,600" fill="#070e07"/>
          ...
          <polygon points="0,600 0,550 100,532 260,548 440,530 640,548 820,534 1000,548 1000,600" fill="#030703"/>
          <path d={FULL_PATH} stroke="rgba(0,255,65,0.2)" strokeWidth="2" fill="none" strokeDasharray="8,5"/>
          {sec && (
            <path d={sec.pathTo} stroke={sec.routeActive} strokeWidth="3" fill="none"
                  strokeDasharray="8,5" filter="url(#pathglow)"/>
          )}
        </svg>
              ...
              >
                {!isOpen && active && (
                  <div className="you-here">
                    <IChevDown size={10} />YOU ARE HERE
                  </div>
                )}
                <div className="marker-pin"><Px name={s.icon} size={18} /></div>
      ...
      <div className="topbar">
        ...
        {isOpen && (
          <button className="tb-map-btn" onClick={() => setScreen(null)}><IArrowLeft /> MAP</button>
        )}
        ...
        <button className="tb-help-btn" onClick={() => setShowHelp(h => !h)} title="Keyboard shortcuts (?)">?</button>
      ...
      {isOpen && (
        <div
          ...
          style={sec ? { '--accent': sec.accent, '--accent-glow': sec.glow, borderTopColor: sec.accent } : {}}
        >
          <div className="popup-header">
            <div className="popup-title">
              {isProfile ? '▶ PROFILE' : sec?.label}
            </div>
            {!isProfile && sec && (
            ...
            <button className="popup-close" onClick={() => setScreen(null)}><IClose size={11} /> CLOSE</button>
          </div>
