...
export function HydroProvider({ children }) {
  ...
  const _toast = (msg) => { setToast(msg); setTimeout(() => setToast(''), 3000); };
  ...
  const addReading = useCallback(async (data) => {
    const r = await apiFetch('/api/hydro/readings', { method: 'POST', body: JSON.stringify(data) });
    if (!r.error) { await loadAll(); _toast('Reading logged'); }
    return r;
  }, [loadAll]);

  const updatePump = useCallback(async (data) => {
    const r = await apiFetch('/api/hydro/pump', { method: 'PUT', body: JSON.stringify(data) });
    if (!r.error) { await loadAll(); _toast('Pump schedule updated'); }
    return r;
  }, [loadAll]);

  const triggerPump = useCallback(async () => {
    const r = await apiFetch('/api/hydro/pump/trigger', { method: 'POST' });
    if (!r.error) { await loadAll(); _toast('Pump triggered manually'); }
    return r;
  }, [loadAll]);

  const addDosing = useCallback(async (data) => {
    const r = await apiFetch('/api/hydro/dosing', { method: 'POST', body: JSON.stringify(data) });
    if (!r.error) { await loadAll(); _toast('Dosing logged'); }
    return r;
  }, [loadAll]);
  ...
  const addPlant = useCallback(async (data) => {
    ...
    if (!r.error) { await loadAll(); _toast('Plant added'); }
    return r;
  }, [loadAll]);

  const updatePlant = useCallback(async (id, data) => {
    const r = await apiFetch(`/api/hydro/plants/${id}`, { method: 'PUT', body: JSON.stringify(data) });
    if (!r.error) { await loadAll(); _toast('Plant updated'); }
    return r;
  }, [loadAll]);
  ...
  return (
    <HydroContext.Provider value={{
      latest, history, pump, pumpLog, dosing, plants, toast, loadAll,
      addReading, updatePump, triggerPump,
      addDosing, deleteDosing,
