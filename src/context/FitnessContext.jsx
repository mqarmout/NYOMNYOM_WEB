...
export function FitnessProvider({ children }) {
  ...
  const updateWorkout = useCallback(
    async (id, data) => {
      const res = await apiFetch("/api/fitness/workouts/" + id, {
        method: "PUT",
        body: JSON.stringify(data),
      });
      if (res.error) {
  ...
  const addSet = useCallback(
    async (workoutId, data) => {
      const res = await apiFetch("/api/fitness/workouts/" + workoutId + "/sets", {
        method: "POST",
        body: JSON.stringify(data),
      });
  ...
  const updateSet = useCallback(
    async (setId, data) => {
      const res = await apiFetch("/api/fitness/sets/" + setId, {
        method: "PUT",
        body: JSON.stringify(data),
      });
      if (!res.error) await loadWorkouts();
  ...
  const addMetric = useCallback(
    async (data) => {
      const res = await apiFetch("/api/fitness/metrics", {
        method: "POST",
        body: JSON.stringify(data),
      });
      if (res.error) {
